...
TEST_CASE("replica state table", "[basic]")
{
    ...
    REQUIRE(std::nullopt != rst.get_data_object_state(LOGICAL_PATH_1, state_type::before));
    REQUIRE(std::nullopt != rst.get_data_object_state(LOGICAL_PATH_1, state_type::after));

    // ensure that information matches
    {
        auto before_tuple = rst.get_data_object_state(LOGICAL_PATH_1, state_type::before);
        REQUIRE(before_tuple);
        auto& before = std::get<0>(*before_tuple);
        for (int i = 0; i < REPLICA_COUNT; ++i) {
    ...
    SECTION("add after entry")
    {
        ...
        auto after_tuple = rst.get_data_object_state(LOGICAL_PATH_1, state_type::after);
        REQUIRE(after_tuple);
        auto& after = std::get<0>(*after_tuple);
    ...
    {
        auto original_before_tmp = rst.get_data_object_state(LOGICAL_PATH_1, state_type::before);
        REQUIRE(original_before_tmp);
        auto& original_before = std::get<0>(*original_before_tmp);

        // set some new info and place in the after slot
        obj.data_id(DATA_ID_2);
        obj.replicas().at(1).size(SIZE_2);
        REQUIRE_NOTHROW(rst.set_data_object_state(LOGICAL_PATH_1, obj, state_type::before));

        auto modified_before_tmp_tuple = rst.get_data_object_state(LOGICAL_PATH_1, state_type::before);
        REQUIRE(modified_before_tmp_tuple);
        auto& modified_before = std::get<0>(*modified_before_tmp_tuple);
    ...
    SECTION("track multiple data objects")
    {
        ...
        {
            auto before_tuple = rst.get_data_object_state(LOGICAL_PATH_2, state_type::before);
            REQUIRE(before_tuple);
            auto& before = std::get<0>(*before_tuple);
            for (int i = 0; i < REPLICA_COUNT; ++i) {
                const auto& state_table_replica = before.replicas().at(i);
        ...
        CHECK_NOTHROW(rst.erase_entry(LOGICAL_PATH_2));
    }

    SECTION("ref_count")
    {
        constexpr int max_ref_count = 5;

        for (int i = rst.reference_count(LOGICAL_PATH_1); i < max_ref_count; ++i) {
            const auto ref_count = rst.reference_count(LOGICAL_PATH_1);
            REQUIRE_NOTHROW(rst.create_entry(obj));
            REQUIRE(ref_count + 1 == rst.reference_count(LOGICAL_PATH_1));
        }

        REQUIRE(max_ref_count == rst.reference_count(LOGICAL_PATH_1));

        for (int i = rst.reference_count(LOGICAL_PATH_1); i > 1; --i) {
            const auto ref_count = rst.reference_count(LOGICAL_PATH_1);
        ...
        REQUIRE(1 == rst.reference_count(LOGICAL_PATH_1));
    }

    // ref_count -> 0
    CHECK_NOTHROW(rst.erase_entry(LOGICAL_PATH_1));
