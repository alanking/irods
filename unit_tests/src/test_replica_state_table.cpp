...
TEST_CASE("logical_locking", "[basic]")
{
    ...
    SECTION("write lock")
    {
        ...
        REQUIRE_NOTHROW(rst::update(DATA_ID_1, locked_replica_number, json{{"data_is_dirty", std::to_string(GOOD_REPLICA)}}));

        SECTION("restore status")
        {
            ill::unlock(DATA_ID_1, locked_replica_number, ill::restore_status);
        ...
        {
            ill::unlock(DATA_ID_1, locked_replica_number, STALE_REPLICA);
    ...
    SECTION("read lock")
    {
        ...
        REQUIRE(0 == ill::unlock(DATA_ID_1, locked_replica_number, ill::restore_status));
