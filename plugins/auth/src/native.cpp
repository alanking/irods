...
namespace irods
{
    ...
    private:
        ...
        {
            irods_auth::throw_if_request_message_is_missing_key(req, {"digest", "user_name", "zone_name"});
            json svr_req{req};
        ...
        json native_auth_client_prepare_auth_check(rcComm_t& _comm, const json& req)
        {
            irods_auth::throw_if_request_message_is_missing_key(req, {"user_name", "zone_name", "request_result"});
            json resp{req};
            ...
            return resp;
        } // native_auth_client_prepare_auth_check

        json native_auth_client_perform_auth_check(rcComm_t& comm, const json& req)
        {
            irods_auth::throw_if_request_message_is_missing_key(
            ...
            resp[irods_auth::next_operation] = irods_auth::flow_complete;
            return resp;
        } // native_auth_client_perform_auth_check
        ...
        json native_auth_server_perform_auth_check(rsComm_t& comm, const json& req)
        {
            irods_auth::throw_if_request_message_is_missing_key(req, {"digest", "zone_name", "user_name"});
            ...
            if (LOCAL_HOST != rodsServerHost->localFlag) {
                ...
                if ('\0' == serverId[0]) {
                    log_auth::info("rsAuthResponse: Warning, cannot authenticate the remote server, no RemoteZoneSID "
                ...
                for (int i = 0; i < RESPONSE_LEN; i++) {
                    if (*cp++ != digest[i]) {
                        THROW(REMOTE_SERVER_AUTHENTICATION_FAILURE,
            ...
            if (strcmp(comm.proxyUser.userName, comm.clientUser.userName) != 0) {
                ...
                comm.proxyUser.authInfo.authFlag = comm.clientUser.authInfo.authFlag = authCheckOut->privLevel;
            }
            ...
            return resp;
        } // native_auth_server_perform_auth_check
#endif
