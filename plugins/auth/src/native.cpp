...
namespace
{
    ...
    auto get_password_from_file() -> std::optional<std::string>
    {
        ...
        if (!password_file_stream.is_open()) {
            ...
            THROW(ec,
                  fmt::format(
                      "Failed to open session token file [{}]. errno:[{}]", password_file_path->c_str(), errno));
        }
    ...
    private:
        ...
        json native_auth_establish_context(rcComm_t& _comm, const json& req)
        {
            irods_auth::throw_if_request_message_is_missing_key(
                req, {user_name_kw, zone_name_kw, "request_result"}
            );
            ...
            if (req.at(user_name_kw).get_ref<const std::string&>() == ANONYMOUS_USER) {
        ...
        {
            irods_auth::throw_if_request_message_is_missing_key(
                req, {"digest", user_name_kw, zone_name_kw}
            );
        ...
        {
            irods_auth::throw_if_request_message_is_missing_key(
                req, {"digest", zone_name_kw, user_name_kw}
            );

            // need to do NoLogin because it could get into inf loop for cross zone auth
            rodsServerHost_t *rodsServerHost;
            auto zone_name = req.at(zone_name_kw).get<std::string>();
            int status =
            ...
            const std::string username = fmt::format(
                    "{}#{}", req.at(user_name_kw).get_ref<const std::string&>(), zone_name);
