...
namespace
{
        ...
        const bool _send_notifications) -> int
    {
        ...
        //const auto unlock_statuses = CREATE_TYPE == _l1desc.openType ? ill::restore_status : STALE_REPLICA;

        if (const int ec = ill::unlock(_replica.data_id(), _replica.replica_number(), unlock_statuses); ec < 0) {
