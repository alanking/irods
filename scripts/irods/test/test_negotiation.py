    ...
    def do_signed_zone_key_mismatch_test_on_consumer(self, configuration_key, configuration_value):
        try:
            ...
            with lib.file_backed_up(paths.server_config_path()):
                ...
                # Attempting to reload the configuration will result in an exception being raised because the server
                # cannot get information from the catalog service provider.
                IrodsController().reload_configuration()

                # Show that the catalog provider can no longer be reached.
                self.admin.assert_icommand(['ils'], 'STDERR', 'ZONE_KEY_SIGNATURE_MISMATCH')
