...
class test_targeting_specific_replica_number__issue_6896(
    ...
    @unittest.skip('The unlink operation is not considered in voting. See #4548.')
    def test_iunreg_n_returns_error_when_requested_replica_votes_0(self):
        pass


    @unittest.skip('The unlink operation is not considered in voting. See #4548.')
    def test_iunreg_n_always_overwrites_requested_replica(self):
        pass


    @unittest.skip('The unlink operation is not considered in voting. See #4548.')
    def test_itrim_n_returns_error_when_requested_replica_votes_0(self):
        pass


    @unittest.skip('The unlink operation is not considered in voting. See #4548.')
    def test_itrim_n_always_overwrites_requested_replica(self):
        pass


    @unittest.skip('itouch does not participate in hierarchy resolution, so this test is moot.')
    def test_itouch_n_returns_error_when_requested_replica_votes_0(self):
        import time
        ...
        try:
            ...
            # Capture the original mtime for the target replica and sleep for 1 second to guarantee that it would
            # update if itouch were to succeed.
            original_replica_1_mtime = lib.get_replica_mtime(self.user, self.logical_path, 1)
            time.sleep(1)

            # Touch the existing replica and confirm that it fails to update the mtime.
            self.user.assert_icommand(['itouch', '-n1', self.logical_path], 'STDERR', 'SYS_REPLICA_INACCESSIBLE')
            self.assertEqual(original_replica_1_mtime, lib.get_replica_mtime(self.user, self.logical_path, 1))
            self.assertEqual(str(1), lib.get_replica_status(self.user, os.path.basename(self.logical_path), 0))
            self.assertEqual(str(1), lib.get_replica_status(self.user, os.path.basename(self.logical_path), 1))
    ...
    @unittest.skip('itouch does not participate in hierarchy resolution, so this test is moot.')
    def test_itouch_n_always_touches_requested_replica(self):
        import time
        ...
        try:
            ...
            # Touch the existing replica and confirm that it updates the mtime.
            self.user.assert_icommand(['itouch', '-n1', self.logical_path])
            self.assertNotEqual(original_replica_1_mtime, lib.get_replica_mtime(self.user, self.logical_path, 1))
            self.assertEqual(str(1), lib.get_replica_status(self.user, os.path.basename(self.logical_path), 0))
            self.assertEqual(str(1), lib.get_replica_status(self.user, os.path.basename(self.logical_path), 1))
