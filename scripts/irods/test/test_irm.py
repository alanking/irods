...
class test_no_accidental_data_loss_on_unlink__issue_8441(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.user_logical_path = "/".join([self.user_collection, self.id() + ".data"])
        self.admin_logical_path = "/".join([self.admin_collection, self.id() + ".data"])
        ...
        self.user_physical_path = os.path.join(
            self.vault_path, os.path.sep.join(self.user_logical_path.split("/")[2:]))
        self.admin_physical_path = os.path.join(
            self.vault_path, os.path.sep.join(self.admin_logical_path.split("/")[2:]))
        ...
        self.user.assert_icommand(["itouch", "-R", self.resource, self.user_logical_path])
        ...
        self.admin.assert_icommand(["itouch", "-R", self.resource, self.admin_logical_path])
    ...
    def assert_unlink_fails(self, owner, executor, collection_permission, object_permission, object_path):
        ...
        # Execute unlink on the data object as the "other" user with irm -f.
    ...
    def test_as_user_with_modify_on_collection_and_modify_on_object(self):
        owner = self.admin
        executor = self.user
        target_object = self.admin_logical_path
    ...
    def test_as_admin_with_modify_on_collection_and_modify_on_object(self):
        owner = self.user
        executor = self.admin
        target_object = self.user_logical_path
        collection_permission = "modify_object"
        object_permission = "modify_object"
        self.assert_unlink_fails(owner, executor, collection_permission, object_permission, target_object)
        # Assert that the physical data was not unlinked.
