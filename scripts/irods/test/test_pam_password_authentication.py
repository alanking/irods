...
class test_configurations(unittest.TestCase):
    ...
    def test_user_cannot_authenticate_with_CS_NEG_REFUSE_even_if_tls_is_configured__issue_8360(self):
        # Back up auth session environment contents and service account client environment contents. This test disables
        # TLS in the server while keeping it configured to demonstrate that pam_password no longer enables TLS itself.
        auth_session_env_backup = copy.deepcopy(self.auth_session.environment_file_contents)
        service_account_environment_file_backup = tempfile.NamedTemporaryFile(
            prefix=os.path.basename(self.service_account_environment_file_path)).name
        shutil.copyfile(self.service_account_environment_file_path, service_account_environment_file_backup)
        try:
            # Disable TLS in the test auth session and the service account client environment.
            lib.update_json_file_from_dict(
                self.service_account_environment_file_path, {"irods_client_server_policy": "CS_NEG_REFUSE"})
            ...
            with core_file.temporary_core_file() as core:
                ...
                self.auth_session.assert_icommand(
                    ["iinit"], "STDERR", "TLS_NOT_ENABLED", input=f"{self.auth_session.password}\n")

        finally:
            self.auth_session.environment_file_contents = auth_session_env_backup
            shutil.copyfile(service_account_environment_file_backup, self.service_account_environment_file_path)
