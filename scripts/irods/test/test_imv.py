...
class test_renaming_collections_with_special_characters__issue_6239(unittest.TestCase):
    ...
    #@unittest.skipIf(test.settings.RUN_IN_TOPOLOGY_FROM_RESOURCE_SERVER, "Checks vault.")
    def test_spaces_in_collection_renamed_by_admin_does_not_orphan_contents(self):
        ...
        source_parent_directory = self.user.get_vault_session_path()
        source_directory = f"{source_parent_directory}/{source_subcollection_suffix}"
        source_file = f"{source_directory}/{data_object_name}"
        destination_parent_directory = self.admin.get_vault_session_path()
        ...
        try:
            ...
            self.user.assert_icommand(["istream", "write", source_data_object], input=contents)
            ...
            self.admin.assert_icommand(["ils", "-lr"], "STDOUT")
            self.admin.assert_icommand(["irm", "-rf", destination_parent_collection])
