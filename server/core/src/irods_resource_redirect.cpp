...
namespace
{
        ...
        float&                   _out_vote )
    {
        ...
        if (!err.ok() || irv::is_equal(vote, irv::vote::zero)) {
        ...
        const std::string&     _oper)
    {
        ...
        for (const auto& root_resc : root_map) {
            ...
            if (!irv::is_equal(vote, irv::vote::zero) && !_key_word.empty() && root_resc.first == _key_word) {
                irods::log(LOG_DEBUG, fmt::format(
        ...
        // Floating point numbers should not be compared with 0, so subtract a really small number from the max_vote
        // to ensure that the vote was something above 0. If nothing voted above 0, this is an error.
        if (irv::is_equal(max_vote, irv::vote::zero)) {
        ...
        if (_file_obj->repl_requested() > -1) {
            ...
            if (irv::is_equal(target_replica->get().vote(), irv::vote::zero)) {
                // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-array-to-pointer-decay)
