...
namespace irods
{
    ...
    auto hash_password(const std::string& _password, const std::string& _salt) -> std::string
    {
        ...
        if (_password.size() > maximum_password_length || _salt.size() > maximum_salt_length) {
            THROW(PASSWORD_EXCEEDS_MAX_SIZE, fmt::format("Cannot derive key from password - password or salt exceeds maximum size [{}].", maximum_password_length));
        }
        ...
        *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD, static_cast<void*>(password_buf), static_cast<std::size_t>(_password.size()));
        *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, static_cast<void*>(salt_buf), static_cast<std::size_t>(_salt.size()));
