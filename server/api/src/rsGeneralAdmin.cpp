...
namespace
{
        ...
        const std::string_view _new_user_type) -> void
    {
        ...
        if (!irods::user::type_is_valid(_current_user_type)) {
            THROW(CAT_INVALID_USER_TYPE, "user type for user is not recognized");
        }
    ...
    if ( strcmp( generalAdminInp->arg0, "modify" ) == 0 ) {
        if ( strcmp( generalAdminInp->arg1, "user" ) == 0 ) {
            ...
            try {
                ...
                // arg4 should be the user type to which the target user will be changed if the
                // modification option is "type".
                throw_if_group_is_changing_to_user_or_user_is_changing_to_group(
                    rsComm, option, current_user_type, generalAdminInp->arg4);
