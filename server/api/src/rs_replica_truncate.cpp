...
namespace
{
    ...
    {
        return nlohmann::json{{"message", _message},
                              {"resource_hierarchy", _replica ? _replica->rescHier : ""},
                              {"replica_number", _replica ? _replica->replNum : -1}};
    } // make_json_output
                                                    ...
                                                    std::string& _hierarchy_out) -> int
    {
        ...
        *_doi_out = nullptr;
        ...
        if (const auto* provided_resc_hier = getValByKey(&_inp->condInput, RESC_HIER_STR_KW); provided_resc_hier) {
            // The weird syntax is due to https://github.com/irods/irods/issues/7472.
...
auto rs_replica_truncate(RsComm* _comm, DataObjInp* _inp, char** _out) -> int
{
    ...
    *_out = nullptr;
    ...
    if (const auto* destination_resource = getValByKey(&_inp->condInput, DEST_RESC_NAME_KW); destination_resource) {
        static const auto msg = fmt::format("{}: {} not supported.", __func__, DEST_RESC_NAME_KW);
        log_api::info(msg);
    ...
    {
        const auto msg = fmt::format("{}: {} not supported.", __func__, DEST_RESC_HIER_STR_KW);
        log_api::info(msg);
        *_out = strdup(make_json_output(msg).dump().c_str());
        return SYS_INVALID_INPUT_PARAM;
    }
    ...
    if (resource_name && replica_number) {
        static const auto msg =
            fmt::format("{}: {} and {} cannot be used together.", __func__, RESC_NAME_KW, REPL_NUM_KW);
        log_api::info(msg);
        *_out = strdup(make_json_output(msg).dump().c_str());
        return SYS_INVALID_INPUT_PARAM;
    }
    ...
    try {
        ...
        if (const auto ec = get_data_object_info_and_resolve_hierarchy(_comm, _inp, &doi, resolved_hierarchy); ec < 0) {
            const auto msg =
                fmt::format("{}: Error occurred getting information for [{}]: {}", __func__, _inp->objPath, ec);
            log_api::error(msg);
            *_out = strdup(make_json_output(msg).dump().c_str());
            return ec;
        }
