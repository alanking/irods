...
namespace
{
    ...
    int replicate_data_object(RsComm& _comm, DataObjInp& _inp, transferStat_t** _stat)
    {
        ...
        try {
            ...
            if (destination_resource_input && std::strlen(destination_resource_input) > 0) {
                ...
                // If the client specified a source resource by name and it is the same as the specified destination
                // resource, this is not allowed because it is confusing to replicate from a resource to itself. Users
                // are therefore not allowed to explicitly instruct the API to do this.
                if (source_resource_input && std::string_view{source_resource_input} == destination_resource_input) {
                    THROW(SYS_NOT_ALLOWED,
                          fmt::format(
                              "User specified identical source resource and destination resource: [{}]. Explicitly "
                              "replicating from a root resource to the same root resource is not allowed.",
                              source_resource_input));
                }

                // If the client specified a replica by number and the replica exists in a resource hierarchy with a root
                // resource that matches the specified destination resource, this is not allowed because this is also
                // a request to replicate from a resource hierarchy to itself.
                if (replica_number_input &&
                    irods::hierarchy_parser{source_hierarchy}.resc_in_hier(destination_resource_input)) {
                    THROW(SYS_NOT_ALLOWED,
                          fmt::format("User specified replica number [{}] and destination resource [{}]. Explicitly "
                                      "replicating from a replica number in a hierarchy whose root resource is the "
                                      "destination root resource is not allowed.",
                                      replica_number_input,
                                      destination_resource_input));
                }
            }
            ...
            // the same replica and it would be destructive to try to open it for read and also overwrite it. The reason
            // this situation is allowed is because the client did not explicitly request for this situation, but the
            // policy enacted by the resource hierarchies produced this result, so it should not be treated as an error.
            //
            // These situations should NOT result in any data movement. However, the expected behavior in a resource
            // hierarchy would be for policy to fire and give the appearance that the replica was updated (namely,
            // replication and compound resource hierarchies should be notified of the situation).
    ...
    int update_all_existing_replicas(RsComm& _comm, const dataObjInp_t& _inp, transferStat_t** _stat)
    {
        ...
        for (const auto& destination_replica : destination_obj->replicas()) {
            ...
            // If the source replica is not good, it cannot be used to update an existing replica.
            if (GOOD_REPLICA != source_replica.replica_status()) {
                log_api::debug("[{}:{}] - Selected source replica in hierarchy [{}] is not good and will overwrite an "
                               "existing replica. Replication is not allowed.",
                               __func__,
                               __LINE__,
                               source_replica.resc_hier());
